...
**Classroom fiction only.** Not live org policy. Do not invent DYA hunt tickets, PIR lists, approval chains, or a site architecture card.
...
| 3 | Forward to IR and leadership notify | SOC | Incident route | **1.5.1** type = incident. **1.5.2** clocks (classroom). **1.5.3** SOC + **IR Sam**; leadership **yes**; approved **ticket**. One sentence for leadership: host, user, process chain, the `.vbs`. Not the hash. Not the Run key. |
...
- **0.7** VT is context on a hash/IP/domain you already have, not a Relations class.
