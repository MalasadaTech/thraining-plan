# Module 0.6.2 – Diamond Model  
## Slide Deck Content

**Target Audience:** SOC Analyst, Threat Hunter, CTI Analyst, Detection Engineer  
**Estimated Delivery Time:** 15 minutes  
**Total Suggested Slides:** 6

---

### Slide 1 – Title Slide
**Title:** Module 0.6.2 – Diamond Model  
**Subtitle:** Shared floor (everyone)  
**Footer:** SOC / Hunter / CTI / DE Training Program

**Speaker Notes:**  
ATT&CK named the behavior. This hour is four corners of the story.

---

### Slide 2 – What this hour is
**Title:** What this hour is

See which part of the story you do **not** have.  
Fill four vertices. Name the weakest.

**Speaker Notes:**  
They do this so they stop guessing the actor when the host and domain are all they have.

---

### Slide 3 – Four vertices
**Title:** Four vertices

**Adversary** — who  
**Capability** — what they used  
**Infrastructure** — IP / domain / mailbox  
**Victim** — host / user / org

**Speaker Notes:**  
A vendor name on a PDF is not Adversary evidence.

---

### Slide 4 – Weakest
**Title:** Weakest vertex

Least evidence.  
That is the next question.  
Not a guess you write as fact.

**Speaker Notes:**  
Walk workstation + encoded PowerShell + domain. Adversary is empty.

---

### Slide 5 – Knowledge Check
**Title:** Knowledge Check

1. Name the four vertices.  
2. What do you do with the weakest vertex?  
3. Encoded PowerShell on a workstation to a domain. Which vertex is usually weakest, and why?

**Speaker Notes:**  
Answers only in the instructor guide.

---

### Slide 6 – Next
**Title:** Next

**0.6.3** Cyber Kill Chain

**Speaker Notes:**  
Same activity, now in time. Not another Diamond vertex.
