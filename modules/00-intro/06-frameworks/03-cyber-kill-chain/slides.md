# Module 0.6.3 – Cyber Kill Chain  
## Slide Deck Content

**Target Audience:** SOC Analyst, Threat Hunter, CTI Analyst, Detection Engineer  
**Estimated Delivery Time:** 15 minutes  
**Total Suggested Slides:** 6

---

### Slide 1 – Title Slide
**Title:** Module 0.6.3 – Cyber Kill Chain  
**Subtitle:** Shared floor (everyone)  
**Footer:** SOC / Hunter / CTI / DE Training Program

**Speaker Notes:**  
Diamond asked which corner is empty. This hour is where in time this row sits.

---

### Slide 2 – What this hour is
**Title:** What this hour is

Place **this** step on the chain.  
Say why it is not the previous or next stage.

**Speaker Notes:**  
They do this so a first payload is not “the whole intrusion.”

---

### Slide 3 – Stages
**Title:** Seven stages

Reconnaissance → Weaponization → Delivery → Exploitation → Installation → Command and Control → Actions on Objectives

**Speaker Notes:**  
Lockheed Martin Cyber Kill Chain. Do not add extra stages.

---

### Slide 4 – This row only
**Title:** This row only

A `.vbs` in email is **Delivery**.  
Not Exploitation — you did not see it run.  
Not Command and Control — no callback.

**Speaker Notes:**  
Reject the neighbor they did not see.

---

### Slide 5 – Knowledge Check
**Title:** Knowledge Check

1. What is the Kill Chain for?  
2. Name the seven stages in order.  
3. A user received a `.vbs` in email. Why is that Delivery, and why is it not Exploitation?

**Speaker Notes:**  
Answers only in the instructor guide.

---

### Slide 6 – Next
**Title:** Next

**3.3.2** External tools

**Speaker Notes:**  
Purpose and when to pick. Not VT Relations.
